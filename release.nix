let
  ...
  haskOverrides = new: old: rec {
    micro-dal-pg = new.callCabal2nix "micro-dal-pg" (ignore ./.) {};
    micro-dal = new.callPackage ./derivations/micro-dal.nix {};
  };
in {
  inherit pkgs;
  packages = { inherit (pkgs.haskellPackages) micro-dal-pg; };
}
